...
class InventoryService:
    ...
    async def stock_report(self) -> list[dict]:
        ...
        stmt = (
            select(Item.item_id, Item.item_code, Item.item_name, Item.item_type, balance)
            .outerjoin(StockLedger)
            .group_by(Item.item_id)
        )
        result = await self.session.execute(stmt)
        return [
            {"item_id": sid, "item_code": code, "item_name": name, "item_type": item_type or "FG", "current_stock": int(stock or 0)}
            for sid, code, name, item_type, stock in result
        ]
    ...
    async def inventory_valuation(self) -> list[dict]:
        ...
        items = (await self.session.execute(select(Item).order_by(Item.item_code))).scalars().all()
        rows = []
        for item in items:
